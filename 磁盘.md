...
du -sh /var/* | sort -rh
# 排序前10，忽略报错，收集到文件
du -h -d 3 --exclude="/mnt" 2>/dev/null | sort -rh | head -n 10 > /tmp/$(date +%Y-%m-%d).txt
```
...
#/              扫描根目录
# 展示结果
dust -T 4 -r -d 3 -n 10 -X /mnt --skip-total /
# 保存到文件
#-p             full-path
#-c             输出不带颜色控制字符
#-P             不显示进度
#-R             For screen readers.
dust -R -p -c -P -r -d 3 -n 10 -X /mnt --skip-total / >"/tmp/$(date +%F).txt" 2>/dev/null
```
